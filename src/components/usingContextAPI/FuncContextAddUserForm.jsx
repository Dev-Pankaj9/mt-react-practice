...
const FuncContextAddUserForm = () => {
  ...
  const { users, handleAddUsers, updateData, handleEditUser, handleUpdateData } = useContext(UserContext);
  ...
  const colors_list = colors_keys.map((color) => ({
    id: `color-${color}`,
    title: color,
    value: color,
    disabled: false,
  ...
  useEffect(() => {
    if (updateData)
      setInputFields({ name: updateData.name, color: updateData.color });
      ...
      <div className="container">
        <form onSubmit={handleSubmitForm}>
          ...
          <div className="form-group form-group-cta">
            <Button
              title={updateData ? "Update" : "Save"}
              type="submit"
              class_names="btn-submit"
            />
            <Button
              title="Clear"
              type="button"
              class_names="btn-clear"
...
export default FuncContextAddUserForm;
